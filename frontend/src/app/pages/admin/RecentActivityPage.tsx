...
interface UIActivityItem {
  ...
  isRead?: boolean; // Thêm trường isRead
  originalLog?: ActivityLog; // Lưu log gốc để xử lý
}
...
const FilterTabs: React.FC<{
  ...
  const filters = [
    ...
    { value: 'student', label: 'Học sinh', icon: <GraduationCap size={16} /> },
    { value: 'payment', label: 'Tài chính', icon: <DollarSign size={16} /> },
    { value: 'class', label: 'Lớp học', icon: <BookOpen size={16} /> },
    { value: 'teacher', label: 'Giáo viên', icon: <UserCheck size={16} /> },
...
const RecentActivityPage: React.FC = () => {
  ...
  const fetchActivities = async (showRefresh = false) => {
    try {
      ...
      setError(null);
      const data = await activityLogApi.getRecentActivities(200);
  ...
  const handleMarkAllAsRead = async () => {
    // Lấy userId từ activity đầu tiên (hoặc có thể lấy từ context)
    if (activities.length === 0) return;
    
    // Giả định userId từ activity đầu tiên
    const firstActivity = activities[0];
    if (!firstActivity.originalLog) return;
    
    const userId = firstActivity.originalLog.userId;
    
    try {
      setMarkingAllRead(true);
      await activityLogApi.markAllAsRead(userId);
  ...
  const counts = {
    ...
    student: activities.filter(a => ['student_enroll', 'student_graduate', 'student_dropout', 'attendance', 'score_update'].includes(a.type)).length,
    payment: activities.filter(a => ['payment', 'tuition_fee'].includes(a.type)).length,
    class: activities.filter(a => ['class_create', 'class_assign', 'schedule_change', 'exam_create'].includes(a.type)).length,
    teacher: activities.filter(a => ['teacher_assign'].includes(a.type)).length,
    unread: activities.filter(a => !a.isRead).length,
  };

  let filteredByType = activities;
  if (filterType === 'student') {
    filteredByType = activities.filter(a => ['student_enroll', 'student_graduate', 'student_dropout', 'attendance', 'score_update'].includes(a.type));
  } else if (filterType === 'payment') {
    filteredByType = activities.filter(a => ['payment', 'tuition_fee'].includes(a.type));
  } else if (filterType === 'class') {
    filteredByType = activities.filter(a => ['class_create', 'class_assign', 'schedule_change', 'exam_create'].includes(a.type));
  } else if (filterType === 'teacher') {
    filteredByType = activities.filter(a => ['teacher_assign'].includes(a.type));
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-4 text-sm">Đang tải hoạt động...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Theo dõi tất cả hoạt động của trung tâm</p>
          </div>
          ...
          <div className="p-5 border-b border-gray-100">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex-1 relative">
                ...
                <input
                  type="text"
                  placeholder="Tìm kiếm theo tiêu đề, nội dung hoặc người thực hiện..."
                  value={searchQuery}

...
const EditTeacherModal: React.FC<EditTeacherModalProps> = ({
  ...
  // Pre-fill khi teacher thay đổi
  useEffect(() => {
    if (teacher) {
      // Tách họ và tên
      const nameParts = teacher.fullName?.trim().split(' ') || [];
      ...
      setFormData({
        ...
        address: {
          details: teacher.address?.details || '',
          ward: teacher.address?.ward || '',
          province: teacher.address?.province || ''
        }
      });

      setPreviewPhoto(getFullImageUrl(teacher.image));
      setPhotoFile(null);
      setErrors({});
    }
  }, [teacher]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    onSave(updatedTeacher, photoFile || undefined);
    setIsSubmitting(false);
  };

  const handleDelete = () => {
    if (teacher && window.confirm('Bạn có chắc muốn xóa giáo viên này?')) {
      onDelete(teacher.id);
    }
  };
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mt-0.5">
                    Cập nhật thông tin và hồ sơ giảng dạy
                  </p>
                </div>
              </div>
                ...
                <div className="mt-3 pt-2 border-t border-indigo-200/50 dark:border-indigo-700/30">
                  <div className="flex items-start gap-2 p-2 bg-white/60 dark:bg-slate-800/40 rounded-lg">
                    ...
                    <p className="text-[10px] text-indigo-600 dark:text-indigo-400 font-medium">
                      Thông tin sẽ được cập nhật vào hệ thống sau khi lưu.
                    </p>
                  </div>
                </div>
              </motion.div>
